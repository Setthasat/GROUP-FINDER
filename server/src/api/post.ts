...
export class postAPI {
    ...
    public async createPost(req: Request, res: Response) {
        ...
        const joinedUsers = await User.find({ userID: { $in: joined } }).select("userID username");
    ...
    public async sendInbox(req: Request, res: Response) {
        const baseResponseInst = new BaseResponse();
        ...
        try {
            const user = await User.findOneAndUpdate(
                { userID },
                { $push: { inbox: { title_inbox, description_inbox } } },
                { new: true }
            );
    ...
    public async deletePost(req: Request, res: Response) {
        ...
        try {
            ...
            const createdBy = await User.findOne({ userID }).select("username");
    ...
    public async joinParty(req: Request, res: Response) {
        const baseResponseInst = new BaseResponse();
        const { postID, userID } = req.body;
        ...
        try {
            ...
            if (post.joined.some((u: any) => u.userID === userID)) {
            ...
            const user = await User.findOne({ userID }).select("userID username");
            if (!user) {
                baseResponseInst.setValue(404, "User not found", null);
                return res.status(404).json(baseResponseInst.buildResponse());
            }
            ...
            baseResponseInst.setValue(200, message, {
    ...
    public async leaveParty(req: Request, res: Response) {
        const baseResponseInst = new BaseResponse();
        const { postID, userID } = req.body;
